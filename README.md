...
- LTX-2.3 model/support files used by the bundled community workflows.
...
- Custom node packs referenced by those workflows, including LTXVideo, KJNodes, VideoHelperSuite, LoRA Manager, MelBandRoFormer, Easy-Use, GGUF, ControlNet Aux, Essentials, Crystools, RMBG, NVIDIA RTX nodes, WhatDreamsCost, and BFSNodes.
...
```text
download_redgraft=true
```

and provide `CIVITAI_API_KEY` to download CivitAI model-version `3250230`. Leave it false/unset to use the stock LTX-2.5 transformer or another checkpoint while retaining the 2.5 text encoders, VAEs, prompt enhancer, duration head, and latent upscalers.
